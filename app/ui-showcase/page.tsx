...
import MenuItem from "@/app/_components/ui/MenuItem";
import Avatar from "@/app/_components/ui/Avatar";
import Chip from "@/app/_components/ui/Chip";
import Card from "@/app/_components/ui/Card";
import Checkbox, { type CheckedState } from "@/app/_components/ui/Checkbox";
import Select from "@/app/_components/ui/Select";
import TimeSegment from "@/app/_components/ui/TimeSegment";
import Spinner from "@/app/_components/ui/Spinner";
import Toggle from "@/app/_components/ui/Toggle";
import NotificationCard from "@/app/_components/ui/NotificationCard";
import ProgressBar from "@/app/_components/ui/ProgressBar";
import CircularProgress from "@/app/_components/ui/CircularProgress";

function Section({ title, desc, children }: { title: string; desc: string; children: ReactNode }) {
  return (
    <section className="rounded-2xl bg-white p-6 shadow-[1px_1px_4px_2px_rgba(0,0,0,0.08)]">
      <h2 className="mb-2 text-heading-l text-gray-900">{title}</h2>
      <p className="mb-4 text-body-m text-gray-500">{desc}</p>
      {children}
    </section>
  );
}
...
export default function UIShowcase() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <h1 className="mb-8 text-center text-heading-xl text-gray-900">UI 컴포넌트 라이브러리</h1>
      <div className="mx-auto flex max-w-4xl flex-col gap-6">
        {/* 1. Dot */}
        <Section title="1. Dot" desc="완료율 상태 표시 닷 — size (sm / md / lg) × color (primary / white / gray)">
        ...
        <Section title="2. Tag" desc="태그 / 배지 — variant × filled / outlined + 아이콘 지원">
          <div className="flex flex-col gap-3">
            <div className="flex flex-wrap items-center gap-2">
              <span className="w-14 text-label-s text-gray-500">filled</span>
              <Tag variant="primary" icon={<Plus size={12} />}>추가하기</Tag>
              {TAG_VARIANTS.map((v) => (
                <Tag key={v} variant={v}>{v}</Tag>
              ))}
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="w-14 text-label-s text-gray-500">outlined</span>
              {TAG_VARIANTS.map((v) => (
                <Tag key={v} variant={v} outlined>{v}</Tag>
              ))}
            </div>
          </div>
        </Section>
        ...
        <Section title="3. Button" desc="버튼 — size × variant × solid / outlined">
          <div className="flex flex-col gap-5">
            {BUTTON_SIZES.map((size) => (
              <div key={size}>
                <span className="mb-2 block text-label-m text-gray-600">{size.toUpperCase()}</span>
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {BUTTON_VARIANTS.map((v) => (
                      <Button key={`${v}-o`} size={size} variant={v} outlined>{v}</Button>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <Section title="6. Chip" desc="칩 — variant × outlined / filled + 아이콘">
          <div className="flex flex-wrap gap-3">
            ...
            <Chip variant="primary" icon={<ArrowLeftRight size={14} />}>주색 filled</Chip>
            <Chip outlined icon={<ArrowLeftRight size={14} />}>기본 outlined</Chip>
            <Chip variant="primary" outlined icon={<ArrowLeftRight size={14} />}>주색 outlined</Chip>
          </div>
        </Section>
        ...
        <Section title="7. Card" desc="콘텐츠 카드 — default / primary">
          <div className="flex gap-4">
            ...
            <Card title="주색 카드" description="주색 연한 배경의 콘텐츠 카드입니다." variant="primary" />
          </div>
        </Section>

        {/* 8. Checkbox */}
        <Section title="8. Checkbox" desc="체크박스 — 클릭하여 상태 전이 (unchecked → checked → unchecked)">
        ...
        <Section title="9. Select" desc="드롭다운 셀렉트 — 기본 / disabled">
          <div className="flex gap-4">
            <div className="w-48">
              <span className="mb-1 block text-label-s text-gray-500">기본</span>
              <Select options={SELECT_OPTIONS} value={selectValue} placeholder="멘티 선택" onChange={setSelectValue} />
            </div>
            <div className="w-48">
              <span className="mb-1 block text-label-s text-gray-500">disabled</span>
              <Select options={SELECT_OPTIONS} value={null} placeholder="멘티 선택" disabled onChange={() => {}} />
            </div>
          </div>
        </Section>
        ...
        <Section title="10. TimeSegment" desc="시간 세그먼트 — 클릭하여 활성 세그먼트 변경">
          <TimeSegment segments={["01", "30", "02", "45"]} activeIndex={timeActive} onSegmentClick={setTimeActive} />
        </Section>
        ...
        {/* 13. NotificationCard */}
        <Section title="13. NotificationCard" desc="알림 카드 — 아이콘 + title + subtitle + 액션 버튼">
          <NotificationCard
        ...
        {/* 15. CircularProgress */}
        <Section title="15. CircularProgress" desc="원형 진행 표시 (점수 링) — 색상은 점수 기준 자동 결정">
          <div className="flex gap-6">
            {[
              { value: 85, color: "초록" },
              { value: 55, color: "노랑" },
              ...
              <div key={item.value} className="flex flex-col items-center gap-2">
                ...
                <span className="text-label-s text-gray-500">{item.value}점 ({item.color})</span>
              </div>
            ))}
          </div>
        </Section>
